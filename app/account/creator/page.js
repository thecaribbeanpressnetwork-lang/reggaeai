...
import ImportRightsControl from '../../components/ImportRightsControl';
...
export default async function CreatorPage() {
  ...
  return <main className="accountShell">
    ...
    <section className={styles.panel}><div className={styles.head}><div><span className="eyebrow">IMPORTS</span><h2>{imports.length} retained source records</h2></div></div>{imports.length?<div className="adminList">{imports.map(i=><div className="payoutRow" key={i.id}><div><strong>{i.source_provider}</strong><span>{i.canonical_url||i.source_url}</span><span>{i.state}</span></div><div><ImportRightsControl importId={i.id} confirmed={Boolean(i.rights_confirmed)}/>{i.hosted?<span>HOSTED</span>:null}{i.monetizable?<span>MONETIZABLE</span>:null}</div></div>)}</div>:<div className="emptyState"><strong>No persistent imports yet.</strong><span>{dbReady?'Use Upload or Smart Import to create source records.':'The importer can discover metadata, but nothing is being pretended as persisted without a database.'}</span><a href="/upload">Open Upload</a></div>}</section>
