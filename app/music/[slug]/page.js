...
export default async function TrackPage({ params }) {
  ...
  let products=[];
  if(databaseConfigured() && persistable) try{const result=await query(`select id,product_type,currency,amount from products where recording_id=$1 and active=true order by amount`,[track.id]);products=result.rows;}catch(error){console.error('track_products_read_failed',error?.message||error);}
