...
const seedArtist = { id:'seed-aven-indigo', slug:'aven-indigo', displayName:'Aven Indigo', artistType:'ai_persona', bio:'Aven Indigo is an AI artist and creative persona blending Caribbean roots, country storytelling and reggae soul. The persona is fictional; no biological-human biography or performance history is asserted.', region:'Caribbean', verified:true, provenanceState:'FOUNDER_VERIFIED' };
const seedRecording = { id:'seed-river', slug:'the-river-knows-my-name', title:'The River Knows My Name', artist:seedArtist, primaryGenre:'Country Reggae', labelName:null, artworkUrl:null, audioUrl:null, durationSeconds:203.964, rightsState:'FOUNDER_CLEARED_SUNO_PRO', publicationState:'RELEASE_READY', aiProvenance:{artistPersona:'AI',lyrics:'ChatGPT generated from founder creative prompt/direction',music:'Suno Pro generated',vocals:'Suno synthetic voice',uploadedAudio:false,thirdPartySamples:false,realPersonVoiceClone:false,source:'https://suno.com/s/qfUnGNAUa1ovUZZx'}, isrc:'TTAJ22600003', isrcState:'RESERVED_FOR_APPROVED_MASTER', masterSha256:'1d393fac0162c0ad49af2f696128d385f706f9bee29de20b48bbaa2806fa3061' };
...
export async function searchPublicCatalogue(rawQuery){
  ...
  const seedMatches=seedSearchItems.filter(item=>item.name.toLowerCase().includes(needle));
  let databaseMatches=[];
  ...
  if(databaseConfigured())try{
    ...
    const[artists,tracks,productions,releases]=await Promise.all([
      ...
      query("select 'release' type,slug,title name from releases where publication_state='PUBLISHED' and title ilike $1 order by release_date desc nulls last limit 8",[like])
    ]);
    databaseMatches=[...artists.rows,...tracks.rows,...productions.rows,...releases.rows].map(item=>({...item,href:item.type==='artist'?`/artists/${item.slug}`:item.type==='track'?`/music/${item.slug}`:item.type==='riddim'?`/riddims/${item.slug}`:`/releases/${item.slug}`}));
  }catch(error){console.error('public_search_failed',error?.message||error);}

  const seen=new Set();
  return [...databaseMatches,...seedMatches].filter(item=>{const key=`${item.type}:${item.slug}`;if(seen.has(key))return false;seen.add(key);return true;});
}
